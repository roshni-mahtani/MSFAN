...
from src.losses import SparseCrossEntropyLoss

from src.data_loader import load_all_data, get_fold_loaders
from src.models import factory_models
from src.utils import calculate_metrics, save_confusion_matrix, plot_loss, save_results_to_file, set_seed, check_gpu, str2bool, save_run_config
...
def train_one_fold(fold_idx, all_data, args, device):
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_loader:
            ...
            if args.mode == 'dual':
                ...
                y = y.to(device, non_blocking=True)
                logits, alpha, _, _ = model(x, return_attention=True)
                outputs = (logits, alpha)
        ...
        with torch.no_grad():
            for batch in val_loader:
                if args.mode == 'dual':
                    ...
                    x = x.to(device, non_blocking=True)
                    y = y.to(device, non_blocking=True)
                    logits, alpha, weights, attn_map = model(x, return_attention=True)
                    outputs = (logits, alpha)
    ...
    alphas_all, weights_all, attn_all = [], [], []
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            alphas_all.append(alpha.cpu())
            weights_all.append(weights.cpu())
            attn_all.append(attn_map.cpu())
    ...
    alpha_mean = torch.cat(alphas_all, dim=0).mean(dim=0)      # (F,)
    alpha_std = torch.cat(alphas_all, dim=0).std(dim=0)
    weights_mean = torch.cat(weights_all, dim=0).mean(dim=0)  # (F,)
    attn_mean = torch.cat(attn_all, dim=0).mean(dim=0)  # (F, F)

    att_dir = os.path.join(args.results_dir, args.name, "attention")
    os.makedirs(att_dir, exist_ok=True)

    np.save(os.path.join(att_dir, f"alpha_fold_{fold_idx}.npy"), alpha_mean.numpy())
    np.save(os.path.join(att_dir, f"alpha_std_fold_{fold_idx}.npy"), alpha_std.numpy())
    np.save(os.path.join(att_dir, f"weights_fold_{fold_idx}.npy"), weights_mean.numpy())
    np.save(os.path.join(att_dir, f"attn_fold_{fold_idx}.npy"), attn_mean.numpy())
            
    metrics, cm = calculate_metrics(np.array(all_targets), np.array(all_preds), np.array(all_probs), n_classes)
    return metrics, cm, train_losses, val_losses, np.array(all_preds), np.array(all_targets), np.array(all_probs)

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--results_dir', type=str, default='./outputs')
    parser.add_argument('--name', type=str, default='MSFAN')
    parser.add_argument('--data_dir', type=str, default='./data')
    
    # Nuevas arquitecturas
    parser.add_argument('--architecture', type=str, default='MultiScaleFeatureAttentionNetwork2')
    ...
    for fold in range(1, 6):
        metrics, cm, t_loss, v_loss, preds, targets, probs = train_one_fold(fold, all_data, args, device)
